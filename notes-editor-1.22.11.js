...
(() => {
  ...
  class Toolbar {
    ...
    render() {
      ...
      for(const [id,name,icon] of [['select','Selectare','select'],['text','Text','text'],['pencil','Desen','pencil'],['shape','Forme','shape'],['image','Imagine','image'],['pdf','PDF','pdf']]){const b=button(name,()=>id==='image'?e.chooseImage():id==='pdf'?e.choosePdf():e.setTool(id),icon);b.dataset.mode=id;b.setAttribute('aria-pressed',String(e.tool===id));this.modes.append(b);}
      this.renderContext();
    }
