(() => {
  ...
  let db=null,items=[],query='',activeCategory=null,objectUrls=[];
  ...
  const all=async()=>((await transact('readonly',store=>store.getAll()))||[]).map(item=>({...item,category:item.category||'pal'})).sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
  ...
  const match=item=>(`${item.name} ${item.width||''} ${item.height||''} ${item.depth||''}`).toLocaleLowerCase('ro-RO').includes(query);
  ...
  function render(){
    ...
    if(activeCategory){
      ...
      announce(`${rows.length} ${rows.length===1?'material':'materiale'} în ${category(activeCategory).name} · salvate pe acest dispozitiv`);
    ...
    if(query){
      const rows=items.filter(match);announce(`${rows.length} rezultate · toate dosarele`);
    ...
    announce('Alege un dosar pentru a vedea materialele salvate.');
  ...
  function makeItemCard(item,showCategory=false){
    ...
    if(showCategory)top.append(el('span','inventory-item-category',category(item.category).name));top.append(title);
    ...
    const actions=el('div','inventory-card-actions'),edit=el('button','','Editează'),del=el('button','inventory-delete','Șterge');edit.type=del.type='button';edit.addEventListener('click',()=>openForm(item,item.category));del.addEventListener('click',async()=>{if(!confirm(`Ștergi „${item.name}” din inventar?`))return;try{await remove(item.id);items=await all();render();announce('Materialul a fost șters.');}catch(error){announce(error.message,true);}});
  ...
  function openForm(item,initialCategory){
    ...
    const name=document.createElement('input');name.type='text';name.maxLength=120;name.required=true;name.placeholder='De exemplu: Stejar Halifax natur';name.value=item?.name||'';const nameField=el('label','inventory-field','Nume material');nameField.append(name);
