(() => {
  ...
  const addButton=document.getElementById('workspaceInventoryAdd'),addFolderButton=document.getElementById('workspaceInventoryAddFolder'),breadcrumb=document.getElementById('workspaceInventoryBreadcrumb');
  ...
  const category=id=>categories.concat(folders).find(item=>item.id===id)||categories[0];
  ...
  const all=async()=>((await transact('readonly',store=>store.getAll()))||[]).map(item=>{const folderId=item.folderId||item.category||'pal';return {...item,category:folderId,folderId};}).sort((a,b)=>timestamp(b.updatedAt)-timestamp(a.updatedAt));
  const allTombstones=async()=>((await transact('readonly',store=>store.getAll(),tombstoneStore))||[]);
  const allFolders=async()=>((await transact('readonly',store=>store.getAll(),'folders'))||[]).sort((a,b)=>a.name.localeCompare(b.name,'ro'));
  ...
  function render(){
    ...
    categories.concat(folders).forEach(group=>{
