...
(() => {
  ...
  class NotesStore {
    async open() {
      ...
      if (!await this.get('meta','legacyImported')) {
        ...
        for (let i=0;i<legacy.length;i++) {
          ...
          tx.objectStore('notes').put({id:`legacy-${i}`,schema:1,title:n.title||'Notiță importată',description:'',createdAt:now(),updatedAt:now(),pages});
        }
    ...
    async put(note) { const tx=this.db.transaction('notes','readwrite'); tx.objectStore('notes').put(copy(note)); await this.done(tx); }
    async remove(id) { const tx=this.db.transaction('notes','readwrite');tx.objectStore('notes').delete(id);await this.done(tx); }
  }
  ...
  class TextObject {
    ...
    static paint(ctx,o) {
      ...
      const rows=[]; for(const line of this.lines(o)){let row='';for(const word of line.split(/(?<=\s)/)){ if(ctx.measureText(row+word).width>o.w-12&&row){rows.push(row);row='';} if(ctx.measureText(word).width>o.w-12){for(const c of word){if(ctx.measureText(row+c).width>o.w-12&&row){rows.push(row);row='';}row+=c;}}else row+=word;} rows.push(row);}
      const step=o.fontSize*1.35;
      rows.forEach((row,i)=>{ const width=ctx.measureText(row.trimEnd()).width, x=o.align==='center'?(o.w-width)/2:o.align==='right'?o.w-width-6:6, y=6+i*step; ctx.fillText(row,x,y); if(o.underline){ctx.fillRect(x,y+o.fontSize+2,width,Math.max(1,o.fontSize/18));} });
    }
  }
  class ImageObject {
  ...
  class A4Canvas {
    ...
    static paint(ctx,page,images,except) {
      this.paper(ctx,page);
      for(const o of page.objects){if(o.id===except)continue;ctx.save();ctx.translate(o.x+o.w/2,o.y+o.h/2);ctx.rotate(o.rotation*Math.PI/180);ctx.translate(-o.w/2,-o.h/2);ctx.globalAlpha=o.opacity??1;
  ...
  class PageManager {
    ...
    add() {const e=this.e,fields=el('div','an-form-grid'),orientation=select([['portrait','A4 vertical'],['landscape','A4 orizontal']],e.page.orientation),paper=select([['blank','Albă'],['ruled','Cu linii'],['grid','Cu pătrățele']],e.page.paper);fields.append(field('Format',orientation),field('Foaie',paper));dialog('Adaugă pagină A4',fields,()=>{e.edit(()=>{e.note.pages.splice(e.pageIndex+1,0,pageData(orientation.value,paper.value));e.pageIndex++;});e.fit();},'Adaugă');}
  ...
  class Toolbar {
    constructor(editor) {this.e=editor;this.node=el('div','an-toolbar');this.modes=el('div','an-modes');this.context=el('div','an-context');this.node.append(this.modes,this.context);this.render();}
    render() {
      const e=this.e;this.modes.replaceChildren();
      for(const [id,name,icon] of [['select','Selectare','select'],['text','Text','text'],['pencil','Creion','pencil'],['marker','Marker','marker'],['shape','Forme','shape'],['image','Imagine','image'],['pan','Deplasare','pan']]){const b=button(name,()=>id==='image'?e.chooseImage():e.setTool(id),icon);b.setAttribute('aria-pressed',String(e.tool===id));this.modes.append(b);}
    ...
    renderContext() {
      const e=this.e,o=e.object;this.context.replaceChildren();
      ...
      if(o&&e.tool==='select') {
        ...
        const opacity=input(Math.round(o.opacity*100),'range');opacity.min=5;opacity.max=100;opacity.disabled=!!o.locked;opacity.addEventListener('change',()=>e.changeObject({opacity:Number(opacity.value)/100}));add('Opacitate',opacity);
      ...
      if((o?.type==='text'&&!o.locked)||e.tool==='text') {
        ...
        for(const [key,name] of [['bold','Bold'],['italic','Italic'],['underline','Subliniat']]){const b=button(name,()=>{change({[key]:!style[key]});if(!o)this.renderContext();});b.setAttribute('aria-pressed',String(!!style[key]));this.context.append(b);}
        ...
        const width=input(e.drawWidth,'range');width.min=1;width.max=40;width.addEventListener('input',()=>{e.drawWidth=Number(width.value);width.title=`${width.value} px`;});add('Grosime',width);
        ...
        add('Formă',select([['rect','Dreptunghi'],['ellipse','Elipsă'],['arrow','Săgeată']],e.shapeKind,v=>e.shapeKind=v));const color=input(e.drawColor,'color');color.addEventListener('input',()=>e.drawColor=color.value);add('Culoare',color);
      } else if(!o) {this.context.append(el('span','an-hint',e.tool==='pan'?'Trage pentru deplasare. Două degete pentru zoom.':'Selectează un obiect pentru editare. Poți muta, roti și redimensiona fiecare element.'));}
    }
  }

  class NoteEditor {
    constructor(manager,note) {
      this.manager=manager;this.note=copy(note);this.pageIndex=clamp(note.activePage||0,0,note.pages.length-1);this.selected=null;this.tool='select';this.drawKind='pencil';this.drawColor='#25384b';this.drawWidth=3;this.shapeKind='rect';this.textStyle=TextObject.create(0,0);this.past=[];this.future=[];this.images=new Map();this.pointers=new Map();this.zoom=1;this.saveChain=Promise.resolve();this.dirty=false;this.revision=0;
      ...
      header.append(button('Notițe',()=>this.close(),'back'),titleBlock,button('Detalii',()=>manager.details(this.note,updated=>{this.edit(()=>Object.assign(this.note,updated));this.title.textContent=this.note.title;}),'edit'),button('Export',()=>this.exportMenu(),'export'));
    ...
    changed(){this.note.updatedAt=now();this.note.activePage=this.pageIndex;this.revision++;this.dirty=true;this.status('Se salvează…');clearTimeout(this.timer);this.timer=setTimeout(()=>this.flush().catch(()=>{}),180);this.requestPaint();}
    ...
    async close(){this.endText();try{await this.flush();}catch{return;}this.abort.abort();this.resize.disconnect();cancelAnimationFrame(this.frame);this.root.remove();document.body.style.overflow=this.oldOverflow;this.manager.editor=null;await this.manager.render();document.getElementById('workspaceAddNote').focus();}
    ...
    setTool(tool){this.endText();this.tool=tool;this.drawKind=tool==='marker'?'marker':tool==='pencil'?'pencil':this.drawKind;this.selected=null;this.refresh(false);}
    ...
    move(event){
      ...
      if(g.kind==='draw'){const batch=event.getCoalescedEvents?.()||[event];for(const item of batch.length?batch:[event]){const q=this.point(item),last=this.preview.points.at(-1);if(Math.hypot(q.x-last.x,q.y-last.y)>.5){if(['line','dashed'].includes(this.preview.kind))this.preview.points=[this.preview.points[0],q];else this.preview.points.push(q);}}this.requestPaint();return;}
    ...
    up(event,cancel=false){
      ...
      if(g.kind==='draw'&&this.preview){this.snapshot();this.page.objects.push(DrawingLayer.normalize(this.preview));this.preview=null;this.changed();this.pages.render();}
      ...
      else if(g.kind==='shape'){this.snapshot();const p=this.point(event);const o={id:uid(),type:'shape',shape:this.shapeKind,x:Math.min(g.start.x,p.x),y:Math.min(g.start.y,p.y),w:Math.max(30,Math.abs(p.x-g.start.x)),h:Math.max(30,Math.abs(p.y-g.start.y)),rotation:0,opacity:1,locked:false,color:this.drawColor,fill:'transparent',width:this.drawWidth};this.page.objects.push(o);this.selected=o.id;this.changed();this.pages.render();}
    ...
    chooseImage(replaceId){const picker=input('','file');picker.accept='image/*';picker.addEventListener('change',async()=>{try{if(!picker.files[0])return;const o=await ImageObject.fromFile(picker.files[0]);this.endText();this.snapshot();const previous=this.page.objects.find(n=>n.id===replaceId);if(previous){previous.src=o.src;previous.crop=o.crop;previous.h=previous.w*o.h/o.w;this.selected=previous.id;}else{const [w,h]=size(this.page);if(o.h>h-120){o.w*= (h-120)/o.h;o.h=h-120;}o.x=(w-o.w)/2;o.y=60;this.page.objects.push(o);this.selected=o.id;}this.tool='select';this.changed();this.refresh();}catch(error){this.status(error.message,true);}});picker.click();}
  ...
  class NotesManager {
    constructor(){this.store=new NotesStore();this.panel=document.getElementById('workspaceNotes');this.list=document.getElementById('workspaceNotesList');this.status=document.getElementById('workspaceNotesStatus');this.add=document.getElementById('workspaceAddNote');this.query='';this.ready=this.init();}
    async init(){
      ...
      try{await this.store.open();this.add.disabled=false;await this.render();}catch(error){this.status.textContent=`${error.message} Originalele nu au fost șterse.`;this.status.classList.add('an-error');}
    }
    details(note,save){const content=el('div','an-form-grid'),title=input(note?.title||''),description=el('textarea');title.required=true;title.maxLength=160;title.placeholder='De exemplu: Măsurători bucătărie';description.value=note?.description||'';description.rows=3;content.append(field('Titlu notiță',title),field('Descriere (opțional)',description));let orientation,paper;if(!note){orientation=select([['portrait','A4 vertical'],['landscape','A4 orizontal']],'portrait');paper=select([['blank','Albă'],['ruled','Cu linii'],['grid','Cu pătrățele']],'blank');content.append(field('Prima pagină',orientation),field('Tip foaie',paper));}else content.append(el('p','an-hint',`Creată ${date(note.createdAt)} · Modificată ${date(note.updatedAt)}`));dialog(note?'Detaliile notiței':'Notiță nouă',content,async()=>{if(!title.value.trim())throw Error('Introdu un titlu.');const next=note?{title:title.value.trim(),description:description.value}:{id:uid(),schema:1,title:title.value.trim(),description:description.value,createdAt:now(),updatedAt:now(),pages:[pageData(orientation.value,paper.value)]};await save(next);},note?'Salvează':'Creează');}
    async render(){if(!this.store.db)return;const token=this.renderToken=uid();const notes=await this.store.all();if(token!==this.renderToken)return;this.list.replaceChildren();this.list.className='an-note-grid';notes.sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt));const filtered=notes.filter(n=>(n.title+' '+n.description+' '+n.pages.flatMap(p=>p.objects.map(o=>o.text||'')).join(' ')).toLocaleLowerCase('ro-RO').includes(this.query));this.status.textContent=`${notes.length} notițe · Salvare automată pe acest dispozitiv`;
      ...
      for(const note of filtered){const card=el('article','an-note-card'),open=button(note.title,()=>this.open(note));open.classList.add('an-note-open');const preview=el('canvas');preview.width=250;preview.height=160;open.prepend(preview);const snippet=note.description||note.pages.flatMap(p=>p.objects.filter(o=>o.type==='text').map(o=>o.text)).join(' ')||'Schițe și imagini pe A4';const actions=el('div','an-card-actions');actions.append(button('Redenumește',()=>this.details(note,async patch=>{Object.assign(note,patch,{updatedAt:now()});await this.store.put(note);await this.render();}),'edit','an-icon'),button('Duplică',async()=>{try{const n=copy(note);n.id=uid();n.title+=' — copie';n.createdAt=n.updatedAt=now();n.pages.forEach(p=>{p.id=uid();p.objects.forEach(o=>o.id=uid());});await this.store.put(n);await this.render();}catch(error){this.status.textContent=error.message;}},'copy','an-icon'),button('Șterge',()=>dialog('Ștergi această notiță?',el('p','',`„${note.title}” va fi ștearsă de pe acest dispozitiv. Exportă înainte o copie dacă vrei să o păstrezi.`),async()=>{await this.store.remove(note.id);await this.render();},'Șterge'),'trash','an-icon an-danger'));
        card.append(open,el('p','an-note-preview',snippet),el('p','an-note-date',`${note.pages.length} pagini · ${date(note.updatedAt)}`),actions);this.list.append(card);
    ...
    import(){const picker=input('','file');picker.accept='.json,.anote.json';picker.addEventListener('change',async()=>{try{if(!picker.files[0])return;const n=JSON.parse(await picker.files[0].text());if(n.schema!==1||typeof n.title!=='string'||!Array.isArray(n.pages)||!n.pages.length||n.pages.length>200)throw Error('Copia nu are un format de notiță A4 valid.');for(const p of n.pages){if(!['portrait','landscape'].includes(p.orientation)||!['blank','ruled','grid'].includes(p.paper)||!Array.isArray(p.objects))throw Error('Pagină invalidă.');p.id=uid();for(const o of p.objects){if(!['text','image','drawing','shape'].includes(o.type)||!['x','y','w','h','rotation','opacity'].every(k=>Number.isFinite(o[k]))||o.w<=0||o.h<=0)throw Error('Obiect invalid.');if(o.type==='image'&&(!/^data:image\/(png|jpeg|webp);base64,/.test(o.src)||!o.crop))throw Error('Imagine invalidă.');if(o.type==='text'&&(typeof o.text!=='string'||!Number.isFinite(o.fontSize)))throw Error('Text invalid.');if(o.type==='drawing'&&(!Array.isArray(o.points)||!o.points.length||o.points.some(p=>!Number.isFinite(p.x)||!Number.isFinite(p.y))))throw Error('Desen invalid.');o.id=uid();}}n.id=uid();n.description=typeof n.description==='string'?n.description:'';n.createdAt=n.updatedAt=now();await this.store.put(n);await this.render();}catch(error){this.status.textContent=`Import nereușit: ${error.message}`;}});picker.click();}
  }
  const manager=new NotesManager();
  window.atelierNotes={ready:manager.ready,flush:()=>manager.editor?.flush()||Promise.resolve(),manager,Export,A4Canvas};
})();
