...
(() => {
  ...
  class NoteEditor {
    ...
    exportMenu(){
      this.endText();
      const d=el('dialog','an-dialog an-export-dialog'),header=el('div','an-export-header'),heading=el('div'),content=el('div','an-export-options');
      heading.append(el('h2','','Exportă notița'),el('p','','Alege formatul potrivit pentru paginile tale.'));
      header.append(heading,button('Închide',()=>d.close(),'close','an-icon an-export-close'));
      const groups=[
        ['Document',[['PDF · toate paginile','PDF','Un fișier cu toate paginile A4',async()=>download(await Export.pdf(this.note),filename(this.note.title)+'.pdf')],['Tipărește A4','A4','Tipărește notița la dimensiune A4',async()=>Export.print(this.note)]]],
        ['Imagini',[['PNG · pagina curentă','PNG','Salvează pagina afișată',async()=>download(await Export.png(this.page),filename(this.note.title)+'-'+pageFilename(this.page,this.pageIndex)+'.png')],['PNG · toate paginile (ZIP)','ZIP','Toate paginile ca imagini separate',async()=>download(await Export.zip(this.note),filename(this.note.title)+'.zip')]]],
        ['Copie de siguranță',[['Copie editabilă','JSON','Păstrează textul, desenele și imaginile editabile',async()=>download(new Blob([JSON.stringify(this.note)],{type:'application/json'}),filename(this.note.title)+'.anote.json')]]]
      ];
      for(const [title,items] of groups){const group=el('section','an-export-group'),grid=el('div','an-export-grid');group.append(el('h3','',title),grid);for(const [name,format,description,action] of items){
        const b=button(name,async()=>{b.disabled=true;this.status('Se pregătește exportul…');try{let saved=true;try{await this.flush();}catch{saved=false;}await action();this.status(saved?'Export pregătit.':'Export pregătit. Salvarea locală trebuie reîncercată.',!saved);}catch(error){this.status(error.message,true);}finally{b.disabled=false;}},null,'an-export-option');
        const copyBlock=el('span','an-export-copy');copyBlock.append(el('strong','',name),el('small','',description));b.replaceChildren(el('span','an-export-format',format),copyBlock);grid.append(b);
      }content.append(group);}
      d.append(header,content);d.addEventListener('close',()=>d.remove());document.body.append(d);d.showModal();
    }
  }
