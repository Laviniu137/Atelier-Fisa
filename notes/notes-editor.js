...
(() => {
  ...
  const icons = {
    ...
    copy:'M8 8h13v13H8ZM3 16V3h13', pen:'m5 19 3-7L17 3l4 4-9 9Z M5 19l4-4', eraser:'m3 14 9-11 9 8-9 10H9Z', more:'M5 12h.01M12 12h.01M19 12h.01', sidebar:'M4 3h16a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1Z M10 3v18M5 7h3M5 11h3M5 15h3', pages:'M5 3h14v18H5ZM8 7h8M8 11h8M8 15h5', focus:'M4 9V4h5M15 4h5v5M20 15v5h-5M9 20H4v-5',
  ...
  class Toolbar {
    ...
    renderContext() {
      ...
      const addPalette=(label,value,change,target=this.context)=>{const wrap=el('div','an-color-field');wrap.setAttribute('role','group');wrap.setAttribute('aria-label',label);const heading=el('span','an-choice-heading',label),current=el('span','an-current-color');current.style.setProperty('--an-current',value);current.setAttribute('aria-label',`Culoare selectată: ${value}`);heading.append(current);wrap.append(heading);const colors=['#111827','#d94a4a','#e88b24','#f2c94c','#3c9b68','#287dc4','#7654b8'];const swatches=el('div','an-color-swatches');const colorNames=['Negru','Roșu','Portocaliu','Galben','Verde','Albastru','Violet'];for(const color of colors){const swatch=button(colorNames[colors.indexOf(color)],()=>change(color),null,'an-color-swatch');swatch.style.setProperty('--an-swatch',color);swatch.style.backgroundColor=color;swatch.setAttribute('aria-pressed',String(color.toLowerCase()===String(value).toLowerCase()));swatches.append(swatch);}const custom=input(value,'color');custom.className='an-color-custom';custom.setAttribute('aria-label',`${label} personalizată`);custom.addEventListener('input',()=>change(custom.value));const customWrap=el('span','an-custom-color');customWrap.style.setProperty('--an-current',value);customWrap.classList.toggle('is-selected',!colors.includes(String(value).toLowerCase()));customWrap.title='Culoare personalizată';customWrap.append(custom);wrap.append(swatches,customWrap);target.append(wrap);};
