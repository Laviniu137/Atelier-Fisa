...
(() => {
  ...
  class SelectionLayer {
    ...
    render() {
      ...
      for(const name of (o.type==='text'?['nw','ne','e','sw','se','w','rotate']:['nw','ne','sw','se','rotate'])){const h=el('button',`an-handle an-${name}`);h.type='button';h.setAttribute('aria-label',name==='rotate'?'Rotește obiectul':`Redimensionează ${name}`);h.dataset.handle=name;h.addEventListener('pointerdown',event=>this.e.beginTransform(event,name));this.node.append(h);}
    ...
    updateRotation(value,snapped=false){const output=this.node.querySelector('.an-rotation-readout');if(!output)return;const normalized=((Math.round(value)%360)+360)%360;output.querySelector('strong').textContent=normalized+'°';output.style.transform=`rotate(${-value}deg)`;output.classList.toggle('is-snapped',snapped);}
  ...
  class Toolbar {
    ...
    renderContext() {
      ...
      if(this.node.classList.contains('an-text-toolbar')){
        ...
        if(textAppearanceGroup){this.context.append(...textAppearanceGroup.children);textAppearanceGroup.remove();}this.context.querySelectorAll('.an-context-history').forEach(h=>this.objectActions.prepend(h));properties?.remove();
  ...
  class NoteEditor {
    constructor(manager,note) {
      ...
      this.footer=el('footer','an-editor-footer');this.pageLabel=el('span');this.zoomLabel=el('span');this.textHelp=el('span','an-text-help');
      this.footer.append(this.pageLabel,this.textHelp,button('−',()=>this.setZoom(this.zoom/1.2),'minus','an-icon'),this.zoomLabel,button('+',()=>this.setZoom(this.zoom*1.2),'plus','an-icon'),button('Șterge pagina',()=>this.pages.remove(),'trash','an-icon an-danger'));
    ...
    down(event,pan=false){
      ...
      if(this.tool==='text'){const existing=this.hit(p);if(existing?.type==='text'&&!existing.locked){this.gesture=null;this.pointers.delete(event.pointerId);this.editText(existing);return;}this.gesture.kind='text';return;}
    ...
    insertPresetText({id='body',fontSize=22,bold=false,italic=false,placeholder='Adaugă text'}){
      this.endText();this.snapshot();const [pageWidth,pageHeight]=size(this.page),wide=['title','heading','heading2'].includes(id),boxWidth=Math.min(pageWidth-72,wide?620:500),existing=this.page.objects.filter(object=>object.type==='text').length,y=clamp(92+(existing%7)*30,36,pageHeight-150),o=Object.assign(TextObject.create((pageWidth-boxWidth)/2,y,placeholder,boxWidth),copy(this.textStyle),{id:uid(),fontSize,bold,italic,textPreset:id,text:placeholder,align:wide?'center':'left',list:'none',runs:[]});
    ...
    beginTransform(event,handle){this.endText();const o=this.object;if(!o||o.locked)return;event.preventDefault();event.stopPropagation();event.target.setPointerCapture(event.pointerId);this.gesture={kind:handle==='rotate'?'rotate':'resize',handle,id:event.pointerId,start:this.point(event),before:copy(o),o,moved:false,snapAngle:null};if(handle==='rotate')this.selection.showRotation(o.rotation);}
    move(event){
    ...
    editText(o,checkpoint=true){if(o.locked)return;if(this.textEditor&&this.selected===o.id){this.textEditor.focus();return;}this.endText();if(checkpoint)this.snapshot();this.selected=o.id;this.tool='select';this.textRange={start:o.text.length,end:o.text.length};const t=el('div','an-text-editor');t.contentEditable='true';t.setAttribute('role','textbox');t.setAttribute('aria-multiline','true');t.setAttribute('aria-label','Textul casetei');t.spellcheck=true;t.dataset.placeholder='Scrie aici…';this.textAutoFit=this.autoFit;this.autoFit=false;this.textEditor=t;this.sheet.append(t);TextObject.fit(o);this.renderTextEditor();
      t.addEventListener('pointerdown',event=>event.stopPropagation());t.addEventListener('beforeinput',event=>{if(event.inputType==='insertParagraph'||event.inputType==='insertLineBreak'){event.preventDefault();this.insertTextContent('\n');}else if(event.inputType==='historyUndo'||event.inputType==='historyRedo'){event.preventDefault();event.inputType==='historyUndo'?this.undo():this.redo();}});
