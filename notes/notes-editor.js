...
(() => {
  ...
  class Toolbar {
    ...
    renderContext() {
      ...
      const addPalette=(label,value,change,target=this.context)=>{const wrap=el('div','an-color-field');wrap.setAttribute('role','group');wrap.setAttribute('aria-label',label);const heading=el('span','an-choice-heading',label),current=el('span','an-current-color');current.style.setProperty('--an-current',value);current.setAttribute('aria-label',`Culoare selectată: ${value}`);heading.append(current);const colors=DEFAULT_COLORS;const swatches=el('div','an-color-swatches');const colorOptions=el('div','an-color-options');colorOptions.append(swatches);const colorNames=['Negru','Roșu','Portocaliu','Galben','Albastru'];for(const color of colors){const swatch=button(colorNames[colors.indexOf(color)],()=>change(color),null,'an-color-swatch');swatch.style.setProperty('--an-swatch',color);swatch.style.backgroundColor=color;swatch.setAttribute('aria-pressed',String(color.toLowerCase()===String(value).toLowerCase()));swatches.append(swatch);}const history=loadColorHistory().filter(color=>!colors.includes(color)).slice(0,4);if(history.length){const historyWrap=el('div','an-color-history');historyWrap.setAttribute('aria-label','Culori folosite recent');for(const color of history){const swatch=button(`Culoare recentă ${color}`,()=>change(color),null,'an-color-swatch an-color-history-swatch');swatch.style.setProperty('--an-swatch',color);swatch.style.backgroundColor=color;swatch.setAttribute('aria-pressed',String(color.toLowerCase()===String(value).toLowerCase()));historyWrap.append(swatch);}colorOptions.append(historyWrap);}const custom=input(value,'color');custom.className='an-color-custom';custom.setAttribute('aria-label',`${label} personalizată`);custom.addEventListener('input',()=>{rememberColor(custom.value);change(custom.value);});const customWrap=el('span','an-custom-color');customWrap.style.setProperty('--an-current',value);customWrap.classList.toggle('is-selected',!colors.includes(String(value).toLowerCase())&&!history.includes(String(value).toLowerCase()));customWrap.title='Alege o culoare personalizată';customWrap.append(custom);wrap.append(heading,colorOptions,customWrap);target.append(wrap);};
  ...
  class NotesManager {
    ...
    async render(){if(!this.store.db)return;const token=this.renderToken=uid();const notes=await this.store.all();if(token!==this.renderToken)return;this.list.replaceChildren();this.list.className='an-note-grid';notes.sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt));const filtered=notes.filter(n=>(n.title+' '+n.projectName+' '+n.pages.flatMap(p=>p.objects.map(o=>o.text||'')).join(' ')).toLocaleLowerCase('ro-RO').includes(this.query));this.status.classList.toggle('an-error',Boolean(this.syncError));this.status.textContent=this.syncError||`${notes.length} notițe · ${this.syncing?'Se sincronizează…':this.connected()?'Sincronizare activă între dispozitive':'Salvare locală · conectează același cont pe PC și tabletă'}`;
      ...
      for(const note of filtered){const card=el('article','an-note-card'),open=button(note.title,()=>this.open(note));open.classList.add('an-note-open');open.removeAttribute('title');const preview=el('canvas');preview.width=250;preview.height=160;open.prepend(preview);const heading=el('div','an-note-card-heading'),title=el('h3','an-note-card-title',note.title),favorite=button(note.favorite?'Elimină de la favorite':'Adaugă la favorite',async()=>{note.favorite=!note.favorite;note.updatedAt=now();await this.store.put(note);await this.render();},'star','an-favorite-button');favorite.setAttribute('aria-pressed',String(Boolean(note.favorite)));if(note.favorite)favorite.classList.add('is-active');heading.append(title,favorite);const pageCount=`${note.pages.length} ${note.pages.length===1?'pagină':'pagini'}`,relation=note.projectName?el('span','an-note-project',note.projectName):null,dateLine=el('p','an-note-date',`Ultima modificare: ${date(note.updatedAt)}`),stats=el('div','an-note-card-stats'),pageStat=el('span','',pageCount),projectStat=el('span','',note.projectName?'Proiect asociat':'Fără proiect asociat');stats.append(pageStat,projectStat);const actions=el('details','an-note-card-menu'),summary=el('summary','','⋯'),menu=el('div','an-note-card-menu-items');summary.setAttribute('aria-label','Acțiuni notiță');menu.append(button('Editează',()=>this.details(note,async patch=>{Object.assign(note,patch,{updatedAt:now()});await this.store.put(note);await this.render();}),'edit'),button('Duplică',async()=>{try{const n=copy(note);n.id=uid();n.title+=' — copie';n.createdAt=n.updatedAt=now();n.pages.forEach(p=>{p.id=uid();p.objects.forEach(o=>o.id=uid());});await this.store.put(n);await this.render();}catch(error){this.status.textContent=error.message;}},'copy'),button('Șterge',()=>dialog('Ștergi această notiță?',el('p','',`„${note.title}” va fi ștearsă de pe acest dispozitiv. Exportă înainte o copie dacă vrei să o păstrezi.`),async()=>{await this.store.remove(note.id);await this.render();},'Șterge'),'trash','an-danger'));actions.append(summary,menu);
        card.append(open,heading,dateLine,...(relation?[relation]:[]),stats,actions);this.list.append(card);
