...
(() => {
  ...
  class Toolbar {
    ...
    renderContext() {
      ...
      const addPalette=(label,value,change,target=this.context)=>{const wrap=el('div','an-color-field');wrap.setAttribute('role','group');wrap.setAttribute('aria-label',label);const heading=el('span','an-choice-heading',label),current=el('span','an-current-color',String(value).toUpperCase());current.style.setProperty('--an-current',value);current.setAttribute('aria-label',`Culoare selectată: ${value}`);heading.append(current);wrap.append(heading);const colors=['#111827','#d94a4a','#e88b24','#f2c94c','#3c9b68','#287dc4','#7654b8'];const swatches=el('div','an-color-swatches');for(const color of colors){const swatch=button(color,()=>change(color),null,'an-color-swatch');swatch.style.setProperty('--an-swatch',color);swatch.style.backgroundColor=color;swatch.setAttribute('aria-pressed',String(color.toLowerCase()===String(value).toLowerCase()));swatches.append(swatch);}const custom=input(value,'color');custom.className='an-color-custom';custom.setAttribute('aria-label',`${label} personalizată`);custom.addEventListener('input',()=>change(custom.value));const customWrap=el('span','an-custom-color');customWrap.style.setProperty('--an-current',value);customWrap.classList.toggle('is-selected',!colors.includes(String(value).toLowerCase()));customWrap.title=`Culoare personalizată: ${value}`;customWrap.append(custom);wrap.append(swatches,customWrap);target.append(wrap);};
      ...
      if(this.node.classList.contains('an-text-toolbar')){
        ...
        const extra=el('div','an-text-extra');const more=button('Mai multe opțiuni text',()=>this.togglePopover(more,'Stil și culoare',()=>secondary),'more','an-icon');more.setAttribute('aria-expanded','false');more.classList.add('an-text-more');const currentColor=o?.type==='text'?o.color:e.textStyle.color;more.style.setProperty('--an-current',currentColor);more.title=`Mai multe opțiuni text · Culoare selectată: ${currentColor}`;extra.append(more);this.context.insertBefore(secondary,this.context.querySelector('.an-history'));this.context.insertBefore(extra,this.context.querySelector('.an-history'));
