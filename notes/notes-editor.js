...
(() => {
  ...
  class NoteEditor {
    constructor(manager,note) {
      ...
      this.pagesToggle=button('Pagini',()=>this.togglePages(),'sidebar','an-pages-toggle');this.pagesToggle.setAttribute('aria-expanded','true');this.pagesToggle.setAttribute('aria-controls','an-note-pages');this.pagesToggle.title='Ascunde paginile';this.focusToggle=button('Mod focus',()=>this.toggleFocus(),'focus','an-icon an-focus-toggle');this.focusToggle.setAttribute('aria-pressed','false');
      this.zoomLabel=el('output','an-header-zoom-value','100%');this.zoomLabel.setAttribute('aria-label','Zoom pagină');const zoomControls=el('div','an-header-zoom');zoomControls.setAttribute('role','group');zoomControls.setAttribute('aria-label','Zoom pagină');zoomControls.append(button('Micșorează pagina',()=>this.setZoom(this.zoom/1.2),'minus','an-icon'),this.zoomLabel,button('Încadrează pagina',()=>this.fit(),'fitPage','an-icon an-fit-page'),button('Mărește pagina',()=>this.setZoom(this.zoom*1.2),'plus','an-icon'));
      header.append(button('Notițe',()=>this.close(),'back'),titleBlock,this.pagesToggle,button('Detalii',()=>manager.details(this.note,updated=>{this.edit(()=>Object.assign(this.note,updated));this.title.textContent=this.note.title;}),'edit'),zoomControls,this.focusToggle,button('Export',()=>this.exportMenu(),'export'));
      ...
      this.footer.append(this.pageLabel,this.textHelp,button('Șterge pagina',()=>this.pages.remove(),'trash','an-icon an-danger'));
