...
(() => {
  ...
  class NotesManager {
    ...
    async init(){
      ...
      this.layoutButtons=[...this.panel.querySelectorAll('[data-notes-layout]')];this.layoutButtons.forEach(item=>item.addEventListener('click',()=>{this.layout=item.dataset.notesLayout;this.layoutButtons.forEach(button=>button.setAttribute('aria-pressed',String(button===item)));this.render();}));
      ...
      try{await this.store.open();this.add.disabled=false;this.importButton.disabled=false;await this.render();this.syncSoon(120);}catch(error){this.status.textContent=`${error.message} Originalele nu au fost șterse.`;this.status.classList.add('an-error');}
    }
