...
(() => {
  ...
  class TextObject {
    ...
    static fit(o) {const canvas=document.createElement('canvas'),ctx=canvas.getContext('2d');ctx.font=`${o.italic?'italic ':''}${o.bold?'700':'400'} ${o.fontSize}px ${o.font}`;const widest=Math.max(0,...this.lines(o).map(line=>ctx.measureText(line).width));o.w=Math.max(42,Math.ceil(widest+10));o.h=Math.max(Math.ceil(o.fontSize*1.35+6),Math.ceil(this.lines(o).length*o.fontSize*1.35+6));return o;}
    static paint(ctx,o) {
  ...
  class Toolbar {
    constructor(editor) {this.e=editor;this.node=el('div','an-toolbar');this.modes=el('div','an-modes');this.contextToggle=button('Opțiuni instrument',()=>this.toggleContext(),'more','an-context-toggle an-icon');this.contextToggle.setAttribute('aria-expanded','false');this.context=el('div','an-context');this.node.append(this.modes,this.contextToggle,this.context);this.render();}
    ...
    closePopover(){if(!this.activePopover)return;this.activePopover.trigger.setAttribute('aria-expanded','false');this.activePopover.node.remove();document.removeEventListener('pointerdown',this.popoverDismiss,true);this.activePopover=null;}
    openPopover(trigger,title,content){this.closePopover();const node=el('section','an-toolbar-popover');node.setAttribute('role','dialog');node.setAttribute('aria-label',title);node.append(el('strong','an-toolbar-popover-title',title),content);this.node.append(node);const host=this.node.getBoundingClientRect(),rect=trigger.getBoundingClientRect(),left=clamp(rect.left-host.left,8,Math.max(8,this.node.clientWidth-300));node.style.left=`${left}px`;node.style.top=`${rect.bottom-host.top+8}px`;trigger.setAttribute('aria-expanded','true');this.activePopover={node,trigger};this.popoverDismiss=event=>{if(this.activePopover&&!node.contains(event.target)&&!trigger.contains(event.target))this.closePopover();};requestAnimationFrame(()=>document.addEventListener('pointerdown',this.popoverDismiss,true));}
    ...
    renderContext() {
      ...
      const addPalette=(label,value,change,target=this.context)=>{const wrap=el('div','an-color-field');wrap.setAttribute('role','group');wrap.setAttribute('aria-label',label);wrap.append(el('span','an-choice-heading',label));const colors=['#111827','#d94a4a','#e88b24','#f2c94c','#3c9b68','#287dc4','#7654b8'];const swatches=el('div','an-color-swatches');for(const color of colors){const swatch=button(color,()=>change(color),null,'an-color-swatch');swatch.style.setProperty('--an-swatch',color);swatch.style.backgroundColor=color;swatch.setAttribute('aria-pressed',String(color.toLowerCase()===String(value).toLowerCase()));swatches.append(swatch);}const custom=input(value,'color');custom.className='an-color-custom';custom.setAttribute('aria-label',`${label} personalizată`);custom.addEventListener('input',()=>change(custom.value));wrap.append(swatches,custom);target.append(wrap);};
      ...
      if((o?.type==='text'&&!o.locked)||e.tool==='text') {
        ...
        const textStyles=group('an-text-style-group');for(const [key,name] of [['bold','Bold'],['italic','Italic'],['underline','Subliniat']]){const b=button(name,()=>update({[key]:!style[key]}));b.classList.add('an-text-style-button',`an-text-style-${key}`);b.setAttribute('aria-pressed',String(!!style[key]));textStyles.append(b);}target.append(textStyles);
