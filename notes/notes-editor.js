...
(() => {
  ...
  function buildColorPanel(value,apply){
    ...
    panel.append(area,hueInput,field('Saturație',sat),field('Luminozitate',light));
    const history=loadColorHistory();if(history.length){panel.append(el('strong','an-color-picker-heading','Culori adăugate'));const recent=el('div','an-color-picker-history');history.forEach((color,index)=>{const swatch=button('Culoare salvată '+(index+1),()=>apply(color),null,'an-color-history-swatch');swatch.style.background=color;swatch.setAttribute('aria-pressed',String(color===value));recent.append(swatch);});panel.append(recent);}panel.append(footer);sync();return panel;
  }
  ...
  class Toolbar {
    ...
    renderContext() {
      ...
      const addPalette=(label,value,change,target=this.context)=>{const wrap=el('div','an-color-field');wrap.setAttribute('role','group');wrap.setAttribute('aria-label',label);const heading=el('span','an-choice-heading',label),current=el('span','an-current-color');current.style.setProperty('--an-current',value);current.setAttribute('aria-label',`Culoare selectată: ${value}`);heading.append(current);const colors=DEFAULT_COLORS;const swatches=el('div','an-color-swatches');const colorNames=['Negru','Roșu','Portocaliu','Galben','Albastru'];for(const color of colors){const swatch=button(colorNames[colors.indexOf(color)],()=>change(color),null,'an-color-swatch');swatch.style.setProperty('--an-swatch',color);swatch.style.backgroundColor=color;swatch.setAttribute('aria-pressed',String(color.toLowerCase()===String(value).toLowerCase()));swatches.append(swatch);}const customWrap=button('Culori personalizate și istoric',()=>this.togglePopover(customWrap,label,()=>buildColorPanel(value,color=>{rememberColor(color);change(color);this.closePopover();})),null,'an-custom-color');customWrap.style.setProperty('--an-current',value);customWrap.setAttribute('aria-expanded','false');customWrap.classList.toggle('is-selected',!colors.includes(String(value).toLowerCase()));wrap.append(heading,swatches,customWrap);target.append(wrap);};
