...
(() => {
  ...
  class Toolbar {
    ...
    renderContext() {
      ...
      const addPalette=(label,value,change)=>{const wrap=el('div','an-color-field');wrap.setAttribute('role','group');wrap.setAttribute('aria-label',label);wrap.append(el('span','an-choice-heading',label));const colors=['#25384b','#111827','#d94a4a','#e88b24','#f2c94c','#3c9b68','#287dc4','#7654b8'];const swatches=el('div','an-color-swatches');for(const color of colors){const swatch=button(color,()=>change(color),null,'an-color-swatch');swatch.style.setProperty('--an-swatch',color);swatch.setAttribute('aria-pressed',String(color.toLowerCase()===String(value).toLowerCase()));swatches.append(swatch);}const custom=input(value,'color');custom.className='an-color-custom';custom.setAttribute('aria-label',`${label} personalizată`);custom.addEventListener('input',()=>change(custom.value));wrap.append(swatches,custom);this.context.append(wrap);};
      if(o&&e.tool==='select') {
  ...
  class NoteEditor {
    ...
    move(event){
      ...
      if(g.kind==='draw'){const batch=event.getCoalescedEvents?.()||[event];let changed=false;for(const item of batch.length?batch:[event]){const q=this.point(item),last=this.preview.points.at(-1);if(Math.hypot(q.x-last.x,q.y-last.y)>.5){changed=true;if(['line','dashed'].includes(this.preview.kind))this.preview.points=[this.preview.points[0],q];else {this.preview.points.push(q);this.paintLiveSegment(last,q,this.preview);}}}if(changed&&['line','dashed'].includes(this.preview.kind))this.requestPaint();return;}
    ...
    up(event,cancel=false){
      ...
      if(g.kind==='draw'&&this.preview){event.preventDefault();this.suppressClickUntil=performance.now()+450;this.snapshot();this.page.objects.push(DrawingLayer.normalize(this.preview));this.preview=null;this.changed();this.pages.render();}
