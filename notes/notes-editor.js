...
(() => {
  ...
  class TextObject {
    ...
    static layout(ctx,o){
      const rows=[];let row=[],width=0,y=6+(o.paragraphBefore||0),paragraph=0;const available=Math.max(12,o.w-12-(o.indent||0));
      const push=(last=false)=>{const height=Math.max(o.fontSize,...row.map(g=>g.style.fontSize))*(o.lineHeight||1.35);rows.push({glyphs:row,width,y,height,last,paragraph});y+=height;row=[];width=0;};
      const chars=this.chars(o);let prefix=()=>{const text=o.list==='bullet'?'• ':o.list==='number'?`${paragraph+1}. `:'';for(const c of text){ctx.font=this.font(o);const advance=ctx.measureText(c).width+(o.letterSpacing||0);row.push({c,style:o,advance});width+=advance;}};prefix();
      for(let i=0;i<chars.length;i++){const g=chars[i];if(g.c==='\n'){push(true);y+=(o.paragraphAfter||0)+(o.paragraphBefore||0);paragraph++;prefix();continue;}ctx.font=this.font(g.style);g.advance=ctx.measureText(g.c).width+(o.letterSpacing||0);
      ...
      }push(true);for(const r of rows){let x=6+(o.indent||0)+(o.align==='center'?(available-r.width)/2:o.align==='right'?available-r.width:0);const spaces=r.glyphs.filter(g=>g.c===' ').length,extra=o.align==='justify'&&!r.last&&spaces?Math.max(0,(available-r.width)/spaces):0;for(const g of r.glyphs){g.x=x;g.y=r.y;x+=g.advance+(g.c===' '?extra:0);}}
  ...
  class SelectionLayer {
    ...
    render() {
      ...
      for(const name of (o.type==='text'?['nw','n','ne','e','sw','s','se','w','rotate']:['nw','ne','sw','se','rotate'])){const h=el('button',`an-handle an-${name}`);h.type='button';h.setAttribute('aria-label',name==='rotate'?'Rotește obiectul':`Redimensionează ${name}`);h.dataset.handle=name;h.addEventListener('pointerdown',event=>this.e.beginTransform(event,name));this.node.append(h);}
    }
  }
  class PageManager {
  ...
  function textAdvancedPanel(editor,tab='spacing'){
    const panel=el('div','an-text-advanced'),tabs=el('div','an-text-tabs'),body=el('div','an-text-panel-body');panel.append(tabs,body);let current=tab;
    const object=()=>editor.object?.type==='text'?editor.object:editor.textStyle;
    const patch=value=>editor.applyTextPatch(value,{live:true,whole:true});
    const number=(name,value,min,max,step,change)=>{const row=el('label','an-setting'),control=input(value,'number');control.min=min;control.max=max;control.step=step;control.setAttribute('aria-label',name);control.addEventListener('input',()=>{if(control.value!==''&&Number.isFinite(+control.value))change(clamp(+control.value,min,max));});row.append(el('span','',name),control);body.append(row);return control;};
    const choice=(name,items,value,change)=>{const control=select(items,value);control.setAttribute('aria-label',name);control.addEventListener('change',()=>change(control.value));body.append(field(name,control));return control;};
    const color=(name,value,change)=>{const row=el('label','an-setting'),picker=input(value||'#25384b','color');picker.setAttribute('aria-label',name);picker.addEventListener('input',()=>change(picker.value));row.append(el('span','',name),picker);body.append(row);};
    const render=()=>{body.replaceChildren();for(const b of tabs.children)b.setAttribute('aria-selected',String(b.dataset.tab===current));const o=object();
      if(current==='spacing'){number('Înălțime rând',o.lineHeight||1.35,.8,3,.05,v=>patch({lineHeight:v}));number('Spațiere litere',o.letterSpacing||0,-2,20,.25,v=>patch({letterSpacing:v}));number('Indent',o.indent||0,0,160,1,v=>patch({indent:v}));number('Înainte de paragraf',o.paragraphBefore||0,0,80,1,v=>patch({paragraphBefore:v}));number('După paragraf',o.paragraphAfter||0,0,80,1,v=>patch({paragraphAfter:v}));number('Rotație °',o.rotation||0,-360,360,1,v=>patch({rotation:v}));number('Opacitate %',Math.round((o.opacity??1)*100),0,100,1,v=>patch({opacity:v/100}));}
      if(current==='effects'){for(const [key,name] of [['shadow','Umbră'],['outline','Contur'],['highlight','Evidențiere'],['glow','Strălucire']]){let fx={color:o.color,opacity:key==='highlight'?.25:.5,blur:6,width:1,x:2,y:2,padding:3,radius:3,...o.effects?.[key]};const update=()=>patch({effects:{...object().effects,[key]:copy(fx)}});const previousCount=body.children.length;const head=el('label','an-setting'),check=input('','checkbox');check.checked=!!fx.enabled;check.addEventListener('change',()=>{fx.enabled=check.checked;update();});head.append(el('strong','',name),check);body.append(head);color('Culoare '+name,fx.color,v=>{fx.color=v;update();});number('Opacitate '+name+' %',fx.opacity*100,0,100,1,v=>{fx.opacity=v/100;update();});for(const [prop,label,max] of key==='outline'?[['width','Grosime contur',12]]:key==='highlight'?[['padding','Padding',24],['radius','Rază',24]]:[['blur','Blur',40],['x','Offset X',40],['y','Offset Y',40]])number(name+' · '+label,fx[prop],prop==='x'||prop==='y'?-40:0,max,1,v=>{fx[prop]=v;update();});const section=el('details','an-effect-section'),summary=el('summary'),settings=el('div','an-text-panel-body');const controls=[...body.children].slice(previousCount);summary.append(controls.shift());settings.append(...controls);section.append(summary,settings);section.open=!!fx.enabled;check.addEventListener('click',event=>event.stopPropagation());check.addEventListener('change',()=>section.open=check.checked);body.append(section);}choice('Amestecare',[['source-over','Normal'],['multiply','Multiply'],['screen','Screen']],o.blendMode||'source-over',v=>patch({blendMode:v}));}

    };
    for(const [id,name] of [['spacing','Spațiere'],['effects','Efecte']]){const b=button(name,()=>{current=id;render();});b.dataset.tab=id;b.setAttribute('role','tab');tabs.append(b);}tabs.setAttribute('role','tablist');render();return panel;
  }
    ...
    renderContext() {
      ...
      const addRange=(label,value,min,max,change,suffix='px',changeEvent='input',target=this.context)=>{const wrap=el(suffix==='px'?'div':'label','an-field an-range-field'),top=el('span','an-field-heading'),name=el('span','',label),out=el('output','an-range-value',`${value} ${suffix}`),range=input(value,'range');range.min=min;range.max=max;range.setAttribute('aria-label',label);range.addEventListener('input',()=>out.textContent=`${range.value} ${suffix}`);range.addEventListener(changeEvent,()=>change(Number(range.value)));top.append(name,out);wrap.append(top,range);
        if(suffix==='px'){
          ...
          const control=el('div','an-width-control'),valueBox=el('div','an-width-value'),manual=input(value,'number'),unit=el('span','an-width-unit','px');manual.min=min;manual.max=max;manual.step=1;manual.setAttribute('aria-label',label);manual.inputMode='numeric';
      ...
      if((o?.type==='text'&&!o.locked)||e.tool==='text') {
        ...
        addPopover('Mărime',`${style.fontSize}px`,null,()=>{const content=el('div','an-size-popover'),stepper=el('div','an-size-stepper'),manual=input(style.fontSize,'number');manual.min=8;manual.max=144;manual.setAttribute('aria-label','Valoare mărime');const set=value=>{const next=clamp(Number(value)||22,8,144);style.fontSize=next;manual.value=next;e.applyTextPatch({fontSize:next},{live:true});};stepper.append(button('Micșorează',()=>set(style.fontSize-1),'minus','an-stepper-button'),manual,button('Mărește',()=>set(style.fontSize+1),'plus','an-stepper-button'));manual.addEventListener('change',()=>set(manual.value));const presets=el('div','an-size-presets');for(const value of [12,14,16,18,22,24,32,48])presets.append(popoverOption(`${value}`,()=>set(value),'',`an-size-preset${value===style.fontSize?' is-selected':''}`));content.append(stepper,presets);return content;},target);
        ...
        const presets=[['body','Corp de text',22,false],['subtitle','Subtitlu',28,true],['title','Titlu',36,true],['heading','Heading 1',44,true],['heading2','Heading 2',32,true],['quote','Citat',22,false]],currentPreset=presets.find(item=>item[2]===style.fontSize&&item[3]===style.bold)?.[0]||'body';
        addPopover('Stil text',presets.find(item=>item[0]===currentPreset)[1],null,()=>{const content=el('div','an-style-popover');for(const [id,label,fontSize,bold] of presets){const option=popoverOption(label,()=>update({fontSize,bold,italic:id==='quote',textPreset:id}),'',`an-style-preview${id===currentPreset?' is-selected':''}`);option.style.fontSize=`${Math.min(fontSize,24)}px`;option.style.fontWeight=bold?'700':'400';content.append(option);}return content;},target);
      ...
      if(e.tool==='select'){this.context.querySelectorAll('.an-context-history').forEach(history=>this.objectActions.append(history));}
      if(this.node.classList.contains('an-text-toolbar')){
        ...
        if(objectGroup&&o){const actions=objectGroup.querySelector('.an-text-actions-group');
          ...
          const secondary=el('div','an-object-properties');secondary.append(textAdvancedPanel(e));const edit=objectGroup.querySelector('.an-text-state-group .an-button'),arrange=objectGroup.querySelector('.an-arrange-trigger');
        ...
        if(textAppearanceGroup){this.context.append(...textAppearanceGroup.children);textAppearanceGroup.remove();}this.context.querySelectorAll('.an-context-history').forEach(h=>this.objectActions.prepend(h));if(!o){const more=button('Proprietăți text',()=>this.togglePopover(more,'Proprietăți text',()=>textAdvancedPanel(e)),'more','an-icon');this.objectActions.prepend(more);}else{const more=this.objectActions.querySelector('[aria-label="Proprietăți obiect"]');if(more)this.objectActions.prepend(more);}
  ...
  class NoteEditor {
    constructor(manager,note) {
      ...
      this.footer.append(this.pageLabel,button('−',()=>this.setZoom(this.zoom/1.2),'minus','an-icon'),this.zoomLabel,button('+',()=>this.setZoom(this.zoom*1.2),'plus','an-icon'),button('Șterge pagina',()=>this.pages.remove(),'trash','an-icon an-danger'));
    ...
    beginTransform(event,handle){this.endText();const o=this.object;if(!o||o.locked)return;event.preventDefault();event.stopPropagation();event.target.setPointerCapture(event.pointerId);this.gesture={kind:handle==='rotate'?'rotate':'resize',handle,id:event.pointerId,start:this.point(event),before:copy(o),o,moved:false};}
    move(event){
    ...
    up(event,cancel=false){
      ...
      else if(g.o?.type==='text'&&!g.o.locked){const stamp=performance.now();if(this.lastTextTap?.id===g.o.id&&stamp-this.lastTextTap.time<380){this.lastTextTap=null;this.editText(g.o);return;}this.lastTextTap={id:g.o.id,time:stamp};}
    ...
    renderTextEditor(){const t=this.textEditor,o=this.object;if(!t||!o)return;Object.assign(t.style,{left:o.x+'px',top:o.y+'px',width:o.w+'px',height:o.h+'px',transform:`rotate(${o.rotation}deg)`,fontFamily:o.font,fontSize:o.fontSize+'px',fontWeight:o.bold?'700':'400',fontStyle:o.italic?'italic':'normal',textAlign:o.align,lineHeight:String(o.lineHeight||1.35),letterSpacing:(o.letterSpacing||0)+'px',paddingLeft:(6+(o.indent||0))+'px',paddingTop:(6+(o.paragraphBefore||0))+'px',opacity:o.opacity});
      t.replaceChildren();const runs=o.runs?.length?o.runs:[{start:0,end:o.text.length,style:{}}];let last=0;const append=(text,style)=>{const span=el('span','',text);span._textStyle=copy(style);Object.assign(span.style,{fontFamily:style.font||o.font,fontSize:(style.fontSize||o.fontSize)+'px',fontWeight:(style.bold??o.bold)?'700':'400',fontStyle:(style.italic??o.italic)?'italic':'normal',textDecoration:(style.underline??o.underline)?'underline':'none',color:style.color||o.color});t.append(span);};for(const r of [...runs].sort((a,b)=>a.start-b.start)){if(r.start>last)append(o.text.slice(last,r.start),{});append(o.text.slice(r.start,r.end),r.style);last=r.end;}if(last<o.text.length)append(o.text.slice(last),{});if(!o.text)t.append(el('br'));}
    readTextEditor(){const o=this.object,t=this.textEditor;if(!o||!t)return;let text='',runs=[];const visit=(n,style={})=>{if(n.nodeType===3){const start=text.length;text+=n.nodeValue;if(text.length>start)runs.push({start,end:text.length,style:copy(style)});return;}if(n.nodeType!==1)return;if(n.tagName==='BR'){text+='\n';return;}const block=['DIV','P','LI'].includes(n.tagName);if(block&&text&&!text.endsWith('\n'))text+='\n';for(const child of n.childNodes)visit(child,n._textStyle||style);};for(const n of t.childNodes)visit(n);if(t.childNodes.length===1&&t.firstChild.tagName==='BR')text='';o.text=text;o.runs=runs;TextObject.fit(o,Math.max(42,size(this.page)[0]-o.x-8));t.style.height=o.h+'px';this.captureTextRange();this.changed();this.selection.render();}
    applyTextPatch(patch,{live=false,whole=false}={}){const o=this.object?.type==='text'?this.object:this.textStyle;if(o.locked)return;if(this.textEditor&&Object.keys(patch).some(k=>['appearance','effects','blendMode','rotation','opacity','list','paragraphBefore','paragraphAfter'].includes(k)))this.endText();this.captureTextRange();const range=this.textEditor&&this.textRange;const inline=['font','fontSize','bold','italic','underline','color'];const fragment=!whole&&range&&range.end>range.start&&Object.keys(patch).every(k=>inline.includes(k));
      ...
      if(o===this.object){TextObject.fit(o,Math.max(o.w,size(this.page)[0]-o.x-8));if(this.textEditor){this.renderTextEditor();this.restoreTextRange();}this.changed();this.selection.render();this.schedulePagesRender();}if(!live)this.toolbar.renderContext();else if(!this.toolbar.activePopover)this.toolbar.renderContext();
    }
    editText(o,checkpoint=true){if(o.locked)return;if(this.textEditor&&this.selected===o.id){this.textEditor.focus();return;}this.endText();if(checkpoint)this.snapshot();this.selected=o.id;this.tool='select';this.textRange={start:o.text.length,end:o.text.length};const t=el('div','an-text-editor');t.contentEditable='true';t.setAttribute('role','textbox');t.setAttribute('aria-multiline','true');t.setAttribute('aria-label','Textul casetei');t.spellcheck=true;this.textEditor=t;this.sheet.append(t);TextObject.fit(o);this.renderTextEditor();
      t.addEventListener('pointerdown',event=>event.stopPropagation());t.addEventListener('beforeinput',event=>{if(event.inputType==='insertParagraph'||event.inputType==='insertLineBreak'){event.preventDefault();document.execCommand('insertText',false,'\n');}else if(event.inputType==='historyUndo'||event.inputType==='historyRedo'){event.preventDefault();event.inputType==='historyUndo'?this.undo():this.redo();}});
      let inputTime=0;t.addEventListener('beforeinput',()=>{if(performance.now()-inputTime>650)this.snapshot();inputTime=performance.now();});t.addEventListener('input',()=>this.readTextEditor());t.addEventListener('paste',event=>{event.preventDefault();document.execCommand('insertText',false,event.clipboardData.getData('text/plain'));});
    ...
    endText(){if(!this.textEditor)return;this.readTextEditor();const t=this.textEditor;this.textEditor=null;this.textRange=null;t.remove();this.selection.render();this.requestPaint();this.pages.render();}
