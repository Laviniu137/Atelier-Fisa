...
(() => {
  ...
  class Toolbar {
    ...
    historyControls(){const e=this.e,history=el('div','an-history an-context-history');history.setAttribute('aria-label','Istoric modificări');const undo=button('Anulează',()=>e.undo(),'undo','an-icon'),redo=button('Refă',()=>e.redo(),'redo','an-icon');undo.disabled=!e.past.length;redo.disabled=!e.future.length;history.append(undo,redo);return history;}
    renderContext() {
      ...
      if((o?.type==='text'&&!o.locked)||e.tool==='text') {
        ...
        widthTrigger.innerHTML=`<span class="an-shape-width-copy"><small>Contur</small><span>Grosime</span></span><span class="an-shape-width-mark" aria-hidden="true" style="border-top-width:${Math.min(e.drawWidth,8)}px"></span><strong>${e.drawWidth} px</strong><span class="an-shape-width-chevron" aria-hidden="true"></span>`;
